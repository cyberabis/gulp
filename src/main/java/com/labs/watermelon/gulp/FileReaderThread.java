		...
		try {
			...
			while ((bytesRead = ch.read(bb)) != -1) {
				if (bytesRead != readChunkSize) {
					synchronized (in) {
						in.getInQ().put(
								new String(Arrays.copyOfRange(barray, 0,
										bytesRead)));
						in.notify();
					}
				} else {
			...
			System.err
					.println("Could not open the file, FIle Not Found Exception");
		} catch (IOException e) {
			System.err.println("Could not read the file, IO Exception");
		} catch (InterruptedException e) {
			System.err.println("Could not read the file, Interupted Exception");
		}
